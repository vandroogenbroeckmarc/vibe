...
class ViBe:
	...
	def __init__(self, device, numberOfSamples = 30, matchingThreshold = 10, matchingNumber = 2, updateFactor = 8.0, neighborhoodRadius = 1):
	...
	def segment(self, image):
		...
		mask = self.segmentation_(image)
		self.update_(image,mask)
		return mask
	...
	def initialize(self, image):
		...
		"""
			...
			random noise  is only added on numberOfSamples-matchingNumber
			samples.(This idea originates from the C implementation by
		...
		self.neighbor_row = torch.randint(-self.neighborhoodRadius, self.neighborhoodRadius+1, (amount,)).to(self.device).type(torch.float)
		self.neighbor_col = torch.randint(-self.neighborhoodRadius, self.neighborhoodRadius+1, (amount,)).to(self.device).type(torch.float)
	...
	def segmentation_(self, image):
		...
		delta = image.unsqueeze(-1).repeat(1,1,1,self.numberOfSamples) - self.historyBuffer
		num_matches = torch.sum(torch.where(torch.sum(torch.abs(delta),0) <= matchingThreshold, self.FG1.unsqueeze(-1).repeat(1,1,self.numberOfSamples), self.BG1.unsqueeze(-1).repeat(1,1,self.numberOfSamples)),dim=-1)
		"""
