 ...
 *
 * Grayscale (8UC1 = 1-channel) ViBe example.
 *
 * Usage (identical to python-from-c++/main.py, minus --color which is a
 * no-op here since this binary IS the grayscale variant):
 *
 *   ViBe_8UC1 [-h] [--color] [--no-display] [--output OUTPUT]
 ...
 *   - Replaced deprecated CV_* enum macros and the legacy C API
 *     (cvWaitKey, cvDestroyAllWindows) with their cv::* counterparts.
 *   - Use cv::COLOR_BGR2GRAY (VideoCapture emits BGR, not RGB).
...
int main(int argc, char** argv) {
  ...
  /* Parameterization of ViBe (1-channel / Manhattan). */
  using ViBeImpl = ViBeSequential<1, Manhattan<1>>;
  ...
  while (decoder->read(frame)) {
    /* VideoCapture decodes to BGR, not RGB. */
    ...
    if (!vibe) {
      ...
      std::cout << "[ViBe_8UC1] Parameters:" << std::endl;
      vibe->print(std::cout);
      std::cout << std::endl;
    }
  ...
  std::cout << "[ViBe_8UC1] Processed " << frames
            << " frames in " << elapsed << "s ("
  ...
  if (opts.benchmark) {
    ...
    vibe_cli::printBenchStats(st, "ViBe_8UC1 / 1-channel / Manhattan<1>");
  }
