...
function [] = libvibeModel_Sequential_SetMatchingThreshold ( model , matchingThreshold )
    
    error_str = 'Error. The matching threshold should be an interger larger than 0.' ;
    assert ( isnumeric ( matchingThreshold ) , error_str )
	assert ( numel ( matchingThreshold ) == 1 , error_str )
	assert ( matchingThreshold > 0 , error_str )
    assert ( floor ( matchingThreshold ) == matchingThreshold , error_str )
