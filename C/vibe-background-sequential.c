...
static inline uint32_t abs_uint(const int i)
{
  return (uint32_t)((i >= 0) ? i : -i);
}
...
{
  /* Numerically equivalent to (sum_of_abs_diffs <= 4.5 * threshold) but uses
     integer arithmetic so the comparison is exact and does not trigger
     implicit int->double conversion warnings. */
  return (2U * (abs_uint(r1 - r2) + abs_uint(g1 - g2) + abs_uint(b1 - b2)) <= 9U * threshold);
}
...
// -----------------------------------------------------------------------------
vibeModel_Sequential_t *libvibeModel_Sequential_New(void)
{
...
int32_t libvibeModel_Sequential_Update_8u_C1R(
  ...
  /* Guard against degenerate inputs in release builds: a zero width or
   * height would underflow the unsigned loop bounds (width-1, height-1)
   * to UINT32_MAX and run for billions of iterations. The asserts above
   * already catch this in debug builds. */
  if (width == 0 || height == 0) {
    return -1;
  }
